...
def def_full_file_reader():                                                                         #mustafa
    ...
    while i <= (len(list_services) - 1):
        if 'SR' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('SR') - 1]) + ' ' * (20 - (list_services[i].index('SR') - 1)) + str(list_services[i][list_services[i].index('SR'):])
        i += 1
...
def def_file_sorter(): # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("SR") + 3):]) # Extracting Out "SR" + [SPACE] from and cast out the string into an integer
        i += 1
    ...
    while True:
        ...
        if total_price > var_discount_3: ### price > 5000
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   SR " + str(round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   SR " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       SR " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 # Round() to flour the float into an interger
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_2_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   SR " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   SR " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       SR " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 # Round() to flour the float into an interger
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_1_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   SR " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
